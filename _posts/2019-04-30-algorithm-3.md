...
原链接文：[https://tomoya92.github.io/2019/04/30/algorithm-3/](https://tomoya92.github.io/2019/04/30/algorithm-3/)
...
补充: 下面这种方法是我上司的解法, 他看到这题首先想到的就是使用对折的方式来写, 思路跟上面第三种一样, 只是每三种解法首先使用插值的方法把偶数串转成奇数串, 然后再处理的, 但上司的解法是直接对折找的, 最后再来判断字符串的奇偶, 然后通过截取的方式拿到最长回文字符串
